...
namespace YoungFit.PL
{
	public partial class KullaniciForm : Form
	{
		...
		private void KullanicininYemekleriniGetir()
		{
			...
			//dgvYediklerim.Columns[4].Visible = false;
		}
		...
		private void dgvYediklerim_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			//dgvArananYemek_SelectionChanged(dgvArananYemek, e2);
			...
			//for (int i = 0; i < dgvArananYemek.Rows.Count; i++)
			//{
			//	if ((Yemek)(dgvArananYemek.Rows[i].DataBoundItem) == secili)
			//	{ dgvArananYemek.Rows[i].Selected = true;
			//		if(secili.Resim!=null)
			//		{
			//			using (MemoryStream ms = new MemoryStream(secili.Resim))
			//			{
			//				pbKullaniciResim.SizeMode = PictureBoxSizeMode.Zoom;
			//				pbKullaniciResim.Image = Image.FromStream(ms);
			...
			//	else
			//	{
			//		dgvArananYemek.Rows[i].Selected = false;
